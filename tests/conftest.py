"""
tests/conftest.py -- Master test configuration and fixtures at root.

Ensures proper sys.path configuration and fixture sharing across all automated test suites.
"""
...
FRAUD_DETECTION_ROOT = REPO_ROOT / "BAF-Fraud-Detection-Kit" / "fraud-detection"
TEST_DB_PATH = Path(__file__).resolve().parent / "test_fraud_api.db"
...
def synthetic_baf_df():
    ...
    logit = (
        -4.5
        + 1.5 * (1 - df["name_email_similarity"])
        - 0.01 * df["credit_risk_score"]
        + 1.2 * (df["prev_address_months_count"] < 0)
        + 1.0 * (df["phone_mobile_valid"] == 0)
        + 0.5 * df["email_is_free"]
    )
